...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
		...
		Node<K, V> *temp = new Node<K, V>(key, key_value.second);
		if(root_ == nullptr) {
			root_ = temp;
		...
		if(y == nullptr) {
			...
			//checking property violations and applying cases 1-3
			if(y->color == RED || root_->color == RED) {
				insert_fixup(x);
			}
		}
		end: ;
    ...
    size_t diameter() const {
		...
		return (size_t)diameter(root_->left) + (size_t)diameter(root_->right) + 1;
    }
...
private:
    ...
    void insert_fixup(Node<K, V> *z) {
		Node<K, V> *y = z->parent;
		while(y->color == RED || root_->color == RED) {
			if(y == nullptr && z->color == RED) {
					z->color = BLACK;
			//y is left child
			}else if(y->parent->key() > y->key()) {
				//case 1
				if(y->parent->right != nullptr && y->parent->right->color == RED) {
					y->color = BLACK;
			...
			//y is right child
				...
				//case 1
				if(y->parent->left != nullptr && y->parent->left->color == RED) {
					y->color = BLACK;
    ...
    void left_rotate(Node<K, V> *x) {
        Node<K, V> *y = x->right;
		Node<K, V> *z = x->right->left;
		
		y->parent = x->parent;
		if(x->parent != nullptr && x->parent->key() > x->key()) {
			y->parent->left = y;
		}else if(x->parent != nullptr && x->parent->key() < x->key()) {
			y->parent->right = y;
		}
    ...
    size_t width(Node<K, V> *node, size_t level) const {
		...
		return width(node->right, level-1) + width(node->left, level-1);
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        for(int i = 1; i <= limit; ++i) {
			sum += i * ((2 * width(node, i-1)) - width(node, i));
		}
		size_t temp = width(node, limit);
		sum += (limit+1) * (2 * temp);
		return sum;
    }
};
